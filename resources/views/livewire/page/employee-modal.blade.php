                    ...
                    @forelse($users as $emp)
                        <tr>
                            ...
                            <td class="px-4 p-2 border">
                                ...
                                <button wire:click="confirmDelete({{ $emp->id }})" class="text-red-500 ml-2">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            @if ($formOpen)
                <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div class="bg-white rounded-lg shadow-lg w-full max-w-4xl">
                        <div class="px-6 py-4 bg-gradient-to-r from-green-600 to-blue-300 rounded-t-lg text-white flex justify-between items-center">
                        ...
                        <div class="p-6">
                            <form wire click:submit.prevent="save">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <!-- Profile Photo Upload -->
                                    <div class="flex flex-col items-center gap-4">
                                        <label class="text-gray-700">Profile Photo</label>
                                        <div class="w-32 h-32 bg-gray-100 rounded-full overflow-hidden flex items-center justify-center cursor-pointer relative">
                                            @if ($avatar)
                                                <img src="{{ $avatar->temporaryUrl() }}" class="w-full h-full object-cover" />
                                            @else
                                                <span class="text-gray-400">Click to upload</span>
                                            @endif
                                            <input type="file" wire:model="avatar" accept="image/*" class="absolute w-full h-full opacity-0 cursor-pointer"/>
                                        </div>
